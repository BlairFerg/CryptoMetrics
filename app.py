...
with st.expander("🤖 Ask the dashboard (LLM on Hugging Face Space)"):
    ...
    # Cool-down to avoid rapid-fire requests (helps avoid 429s)
    if "last_ask_ts" not in st.session_state:
        st.session_state.last_ask_ts = 0.0
    now_ts = _t.time()
    cooldown = 4.0

    def normalize_space_target(s: str):
        s = s.strip()
        if s.startswith("http"):
            return s, True   # direct .hf.space URL
        return s, False      # owner/space id
    ...
    def summarize_for_llm(metrics, sig_df, hist_df, n=30):
        cols = [c for c in ["btc_dominance","usdt_dominance","stable_dominance",
                            "alt_mcap_usd","btc_mcap_usd","eth_mcap_usd","dxy"]
                if c in hist_df.columns]
        tail = hist_df.reset_index()[["ts", *cols]].tail(n)

        def nn_last(s):
            s = s.dropna();  return s.iloc[-1] if not s.empty else None
        def nn_first(s):
            s = s.dropna();  return s.iloc[0] if not s.empty else None
    ...
    # --- API discovery & safe predict ---
    def detect_api_and_schema(client: Client):
        """
        Returns (api_name, num_inputs). For ChatInterface we expect 2 inputs (message, history).
        """
        try:
            info = client.view_api()
            endpoints = info.get("endpoints", []) if isinstance(info, dict) else []
            # Prefer /chat, then /predict, then /generate
            for preferred in ["/chat", "/predict", "/generate"]:
                for e in endpoints:
                    if e.get("api_name") == preferred:
                        # In Gradio 4, 'inputs' carries components; len(inputs) is arity
                        num_inputs = len(e.get("inputs", []))
                        return preferred, num_inputs or 1
        except Exception:
            pass
        # Default guess: ChatInterface with 2 inputs
        return "/chat", 2

    def _predict_blocking(target, api_name_hint, prompt, out_q):
        try:
            client = Client(target, verbose=False)
            api_name, num_inputs = detect_api_and_schema(client) if api_name_hint in ("", "(auto)") else (api_name_hint, None)

            # Build args based on arity
            if num_inputs is None:
                # if we forced an api without schema, try both shapes
                try:
                    result = client.predict(prompt, api_name=api_name)  # single arg
                except Exception:
                    result = client.predict(prompt, [], api_name=api_name)  # message + empty history
            else:
                if num_inputs >= 2:
            ...
            out_q.put(("ok", result, api_name, num_inputs))
        except Exception as e:
            out_q.put(("err", str(e), api_name_hint, None))
        ...
        if not question.strip():
            ...
            with st.spinner("Contacting the Space…"):
                ...
                worker = threading.Thread(target=_predict_blocking,
                                          args=(space_target, manual_api, prompt, q),
                                          daemon=True)
                worker.start()
                ...
                if worker.is_alive():
                    ...
                    st.info("Tip: open the Space URL in a browser to warm it, then retry.")
                else:
                    status, payload, used_api, nin = q.get()
                    if status == "ok":
                        st.success(f"Space responded (api: {used_api}, inputs: {nin}) in {_t.time()-t0:.1f}s")
                        st.markdown("**Answer**")
                        st.write(payload)
                    else:
